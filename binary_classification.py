...
def save_metrics(y_test, preds, binary=True):
    metrics = {
...
def get_metrics(clf, X_train, X_test, X_test_full, y_train, y_test, y_test_full, metrics_filename, model_name, dict_letter, dataset):
    ...
    result += 'train\n'
    result += f"{save_metrics(y_train.to_list(), list(predicted))}\n\n"
...
input_data_path = ['tabular_data/CIFAR10/merged_data_all_jsma_binary_attacks.csv',
                   'tabular_data/CIFAR10GRAY/merged_data_all_jsma_binary_attacks.csv',
                   'tabular_data/MNIST/merged_data_all_jsma_binary_attacks.csv']
